/*BANNER WRAPPER 03/19/2024 20:20:30 */
...
jpLog = function(message, args) {
	if(DEBUG) {
		...
		if(typeof message == "string") {
			if(!args) {
				console.log("[JETPACK] (64970): " + message + " : " + clean);
			} else {
				console.log("[JETPACK] (64970): " + message, args);
			}
		} else {
			console.log("[JETPACK] (64970): OBJECT", message);
		}
	}
};
...
try {
  jpMainDocs[64970] = useIsolated ? window.top.document : window.document;
  jpMainWindows[64970] = useIsolated ? window.top : window;
} catch(ex) {
  jpMainDocs[64970] = window.document;
  jpMainWindows[64970] = window;
...
function getDeviceTypeCSID(csid) {
    ...
    var TABLETCS = -8000;
    var MOBILECS = -9000;
    switch (jpMainWindows[64970].JPGetDeviceType()) {
...
if(typeof jpUnloadUnits64970 == "undefined") {
	jpUnloadUnits64970 = function() {
		// Unset the loaded variable
		loadedJp_64970 = null;
		delete loadedJp_64970;
		...
		if(typeof JP != "undefined") {
		    ...
		    JP("[id^=jpplatform_64970]").each(function() {
		    	if(JP(this).html().length > 0) {
		    		var f = JP(this).attr("id").match(/\d+$/);
		 			JP("#jpiframe_64970_" + f).remove();
...
window.jpUnloadUnits = jpUnloadUnits64970;
mainWindow.jpUnloadUnits = jpUnloadUnits64970;
...
var scriptSource = jpGetScriptSource(64970, "https://ads.jetpackdigital.com/");

(function(scripts) {
    var scripts = document.getElementsByTagName('script');
    var s = scripts[scripts.length - 1];

    if(!s || typeof s.getAttribute == "undefined" || !s.getAttribute('src') || !s.getAttribute('src').match(/^(http|https):\/\/(?!.*\?.*?\.jetpackdigital).*?\.jetpackdigital/) || s.getAttribute('src').indexOf("64970") == -1) {
        for(var i = scripts.length; i > 0; i--) {
                s = scripts[scripts.length - i];
                if(s && typeof s.getAttribute != "undefined" && s.getAttribute('src') && s.getAttribute('src').match(/^(http|https):\/\/(?!.*\?.*?\.jetpackdigital).*?\.jetpackdigital/) && s.getAttribute('src').indexOf("64970") > -1) {
                        break;
                } else if(s && typeof s.getAttribute != "undefined" && s.getAttribute('src') && s.getAttribute('src').match(/^(http|https):\/\/(?!.*\?.*?\.jetpackdigital).*?\.jetpackdigital/) && s.getAttribute('src').indexOf("64970") > -1) {
                         return s.getAttribute('src').substr(s.getAttribute('src').indexOf('https://ads.jetpackdigital.com/lineitems/64970'));
                } else {
...
/* IGNORE IBUST:  */
    ...
    try {
        ...
        jpplatdiv.id = "jpplatform_64970_" + fid;
        jpplatdiv.rel = "jpplatform_64970_" + fid;
        jpplatdiv.className = "jpplatform_64970";
        jpplatdiv.fid = fid;
...
function jpLoadJQuery_64970() {
        ...
        if((typeof(jQuery) == 'undefined' || ((parseInt(jQuery.fn.jquery.replace(/\./g,"")) < 99)?parseInt(jQuery.fn.jquery.replace(/\./g,""))*10:parseInt(jQuery.fn.jquery.replace(/\./g,""))) < 172) && typeof(JP) == 'undefined'){
               	if(typeof(loadingJQuery) == 'undefined' || !loadingJQuery) {
        			...
        			setTimeout(function() { jpLoadJQuery_64970(); }, jptimeout);
        		}
...
var jpCreativeSetPercentages64970 = new Array();
var jpCreatives64970 = new Array();
function jpAddPercentages64970(id, percentage) {
	for(i = 0; i < percentage; i++) {
		jpCreativeSetPercentages64970.push(id);
	}
}

function jpGetPercentageUnit64970() {
	var randomIndex = Math.floor(Math.random() * jpCreativeSetPercentages64970.length);
	return jpCreativeSetPercentages64970[randomIndex];
}
...
function jpLoadCreativeSet(cs_id, companion, classname) {
	...
	var innerScriptUrl = "https://ads.jetpackdigital.com/lineitems/64970/" + companion + "/jpdli" + ext + ".js";
...
if(!jpMainWindows[64970].JPGetDeviceType) {
    jpMainWindows[64970].JPGetDeviceType = function() {
...
function getCreativeSets() {
    ...
    creative_sets[0] = {
        id: 0,
        customunit_jsclassname: 'bannerunit_687_13512_64970',
...
function jpRunUnits64970() {
    ...
    jplineitemvariables[64970] = {}
	jplineitemvariables[64970]["Custom Unit Classname"] = "bannerunit_687_13512_64970";
	...
	creative_sets = getCreativeSets();
      	...
      	try{
            ...
            if(typeof bannerTopWindow == "undefined") {
                if(!document.getElementById("jpplatform_64970_" + fid)) {
                    document.write("<div id='jpplatform_64970_" + fid + "' fid='" + fid + "' class='jpplatform_64970' rel='jpplatform_64970_" + fid + "'></div>");
                    if(!document.getElementById("jpplatform_64970_" + fid)) {
                        // Didn't write it, so let's find ourselves, and append to where we are
                        var scriptElement = jpGetScriptElement("64970",".jpplatform_64970");
                        var platformWrapper = document.createElement("div");
                        platformWrapper.id = "jpplatform_64970_" + fid;
                        platformWrapper.fid = fid;
                        platformWrapper.className = "jpplatform_64970";
                        platformWrapper.rel = "jpplatform_64970_" + fid;
